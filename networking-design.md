...
The following are network design decisions for data centers without a {{site.data.keyword.tg_full_notm}} service pattern.
...
-  Bandwidth consumption: Depending on your service provider, data transfer through the VPN might incur usage charges.
...
-  Scalability: Ensure that your chosen VPN solution can accommodate future growth in network size and data transfer needs.
...
- VPN capabilities: If secure remote access is needed, ensure that the firewall supports VPN protocols suitable to requirements.
- Content filtering: Control access to specific websites or categories based on user groups or policies.
- Throughput: Ensure that the firewall can handle your internet traffic volume without bottlenecks.
...
- Budget: Firewalls range in price depending on features and capabilities. Determine a budget and prioritize.
...
Generic Routing Encapsulation (GRE) tunnels support the Bring Your Own IP (BYOIP) requirement.

![illustrates the details of GRE for a non-TGW solution architecture](GRE.svg){: caption="Figure 1. Non-TGW GRE encapsulation" caption-side="bottom"}
...
- Cost: Direct Link Connect is more cost effective, especially for lower bandwidth needs.
...
{{site.data.keyword.dl_short}} Dedicated is ideal for the following:

- Organizations with colocation facilities near {{site.data.keyword.cloud_notm}} Point of Presence (PoPs) or data centers.
...
- Organizations that need fine-grained control over routing and traffic management. Direct link that is dedicated is ideal for moderate bandwidth needs, multi-cloud, or hybrid cloud solution strategies.
...
For more information, see [Direct Link Dedicated locations](/docs/dl?topic=dl-locations#dedicated-locations).

### Direct Link Connect
{: #direct-link-connect}

{{site.data.keyword.dl_short}} Connect is ideal for the following:

-   Organizations with diverse connectivity requirements including multi-cloud and hybrid cloud.
-   Organizations that are not located near {{site.data.keyword.cloud_notm}} PoPs or that need lower bandwidth.
...
For more information, see [Direct Link Connect location and providers](/docs/dl?topic=dl-locations#connect-locations).

## Load balancing
{: #load-balancing}

Review the following information about the three types of load balancing: local load balancing, third party load balancing, and global load balancing. 
...
Local load balancing considerations include the following:
...
For more information, see [Exploring IBM Cloud load balancers](/docs/citrix-netscaler-vpx?topic=citrix-netscaler-vpx-explore), [about {{site.data.keyword.loadbalancer_full}}](/docs/loadbalancer-service?topic=loadbalancer-service-about-ibm-cloud-load-balancer), and [About Citrix Net scaler VPX](/docs/citrix-netscaler-vpx?topic=citrix-netscaler-vpx-about-citrix-netscaler-vpx).

### Third party load balancer appliance
{: #third-party-load-balancer}

If there is a specific need or function that is not offered by any of the load balancer options within the {{site.data.keyword.cloud_notm}} catalog, or an existing relationship is established with a third-party vendor, such as F5, you can deploy it within {{site.data.keyword.cloud_notm}} as a bring your own appliance.

## Global load balancing
{: #global-load-balancing}

Global Server Load Balancing (GSLB) is a technique for distributing internet traffic across geographically dispersed servers. It aims to optimize user experience and application performance by directing users to the nearest or most appropriate server based on various factors like latency, server load, and user location. It's a valuable component of a disaster recovery strategy.

If GSLB is required, {{site.data.keyword.cloud_notm}} offers a global load balancer service through {{site.data.keyword.cis_full_notm}}, which routes traffic to servers across multiple geographic locations to ensure application availability.

Other third-party connectivity providers such as Akamai, network appliances from vendors like F5 and Citrix, as well as Domain Name Service can also be used to meet the requirement for GSLB.
...
In {{site.data.keyword.cloud_notm}} Classic, Virtual route forwarding (VRF) must be enabled on the account to move routing to a dedicated routing table. With VRF enabled, Private service endpoints can be enabled, and cloud service resources can be provisioned and accessible to the {{site.data.keyword.cloud_notm}} private network from an {{site.data.keyword.IBM_notm}} assigned IP address in the Classic account.

For more information, see [{{site.data.keyword.cloud_notm}} VRF](/docs/dl?topic=dl-overview-of-virtual-routing-and-forwarding-vrf-on-ibm-cloud).

When the source address is not an {{site.data.keyword.IBM_notm}} assigned IP address a proxy server in classic is used as an intermediary allowing access to cloud services over the Direct Link.

Verify that private service endpoints are available for your cloud services. For more information, see [Enabling VRF and service endpoints](/docs/account?topic=account-vrf-service-endpoint&interface=ui) and [getting started with service endpoints](/docs/account?topic=account-vrf-service-endpoint&interface=ui).
{: note}

In a multi-region deployment, which is described later in this document, for nontransit gateway locations, often a secondary region, which does have transit gateway enabled can be chosen. In these situations, as an alternative to private service endpoints and a proxy server in Classic, configuring a {{site.data.keyword.vpe_full}}, complementary {{site.data.keyword.vpc_short}} Service in the 2nd region, can provide access to{{site.data.keyword.cloud_notm}} Services over the private network. Verify that cloud services are {{site.data.keyword.vpe_short}} [enabled](/docs/vpc?topic=vpc-vpe-supported-services).

![illustrates the details of SE vs {{site.data.keyword.vpe_short}} for a non-TGW solution architecture](SE-vs-VPE.svg){: caption="Figure 2. Non-TGW Service Endpoint access" caption-side="bottom"}
...
{: #DNS}
 ...
 {{site.data.keyword.dns_short}} provides access to your systems and services by user-friendly domain names rather than IP addresses.

Key considerations are:

- Public vs private DNS name resolution: The ability to provide DNS-based access to users on the public internet instead of internal DNS-based communication within your private environment.
...
- Public DNS name resolution: Translates human-friendly names into computer-readable addresses.
    - Use the [DNS interface](/docs/dns?topic=dns-how-to-use-the-dns-interface) in the {{site.data.keyword.cloud_notm}} console to manage your zones and records. The domain can be hosted by any 3rd party provider. Set the name server (NS) record to the provided IBM name servers.
...
- Private DNS name resolution: Allows integration with your on-premises DNS server over private connectivity.
    - Configure your own DNS services in the classic environment, typical options include:
        - Install your own custom DNS service on a virtual server.
        - Configure DNS on your gateway device.
    - Use {{site.data.keyword.dns_full_notm}} (complementary VPC Service). 
